...
def test_demo_fixture_in_wheel_and_sdist(tmp_path: Path) -> None:
    ...
    with ZipFile(wheel) as zf:
        assert any(name.endswith("caskmcp/assets/demo/sample.har") for name in zf.namelist())

    with tarfile.open(sdist, "r:gz") as tf:
        assert any(name.endswith("caskmcp/assets/demo/sample.har") for name in tf.getnames())
