"""Tests for cask init — project detection, config generation, and initialization."""
...
def test_detect_existing_caskmcp(tmp_path: Path) -> None:
    (tmp_path / ".caskmcp").mkdir()
    result = detect_project(tmp_path)
    assert result.has_existing_caskmcp is True


def test_detect_no_existing_caskmcp(tmp_path: Path) -> None:
    result = detect_project(tmp_path)
    assert result.has_existing_caskmcp is False
...
def test_generate_config_basic() -> None:
    from caskmcp.core.init.detector import ProjectDetection
...
def test_gitignore_entries() -> None:
    ...
    assert any("CaskMCP" in e for e in entries)
