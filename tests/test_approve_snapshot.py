...
from caskmcp.core.enforce import ConfirmationStore, DecisionEngine
from caskmcp.models.decision import DecisionContext
...
def test_approve_promotes_pending_lockfile(tmp_path: Path) -> None:
    toolpack_file = write_demo_toolpack(tmp_path)
    pending_lockfile = toolpack_file.parent / "lockfile" / "caskmcp.lock.pending.yaml"
    approved_lockfile = toolpack_file.parent / "lockfile" / "caskmcp.lock.yaml"

    runner = CliRunner()
    result = runner.invoke(
        cli,
        ["approve", "tool", "--all", "--lockfile", str(pending_lockfile)],
    )
    ...
    assert toolpack["paths"]["lockfiles"]["approved"] == "lockfile/caskmcp.lock.yaml"
...
def test_approve_seeds_toolpack_trust_store(tmp_path: Path) -> None:
    ...
    seeded_root = toolpack_file.parent / ".caskmcp"
