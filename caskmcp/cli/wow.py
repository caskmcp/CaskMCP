...
def run_prove_smoke(
    *,
    ...
    workdir = _prepare_out_dir(out_dir=out_dir, keep=keep, prefix="caskmcp-prove-smoke-")
...
def _run_offline_wow(*, out_dir: str | None, keep: bool, verbose: bool) -> int:
    ...
    workdir = _prepare_out_dir(out_dir=out_dir, keep=keep, prefix="caskmcp-wow-")
...
def _run_live_wow(
    *,
    ...
    workdir = _prepare_out_dir(out_dir=out_dir, keep=keep, prefix="caskmcp-wow-live-")
    try:
        import playwright  # noqa: F401
    except ImportError:
        click.echo(
            "Error: live mode requires Playwright. Install with: pip install \"caskmcp[playwright]\"",
            err=True,
        )
        return 1
    ...
    try:
        try:
            run_mint(
                ...
                name=f"CaskMCP wow live ({scenario})",
...
def _execute_wow_contract(*, workdir: Path, toolpack_file: Path, scenario_label: str) -> int:
    ...
    lock_manager.load()
    prior_root = os.environ.get("CASKMCP_ROOT")
    os.environ["CASKMCP_ROOT"] = str((workdir / ".caskmcp").resolve())
    try:
        lock_manager.approve_all(approved_by="wow")
        lock_manager.save()
    finally:
        if prior_root is None:
            os.environ.pop("CASKMCP_ROOT", None)
        else:
...
def _run_governed_dry_replay(
    *,
    ...
    gateway = EnforcementGateway(
        ...
        confirmation_store_path=str(workdir / ".caskmcp" / "state" / "confirmations.db"),
        unsafe_no_lockfile=False,
    )
