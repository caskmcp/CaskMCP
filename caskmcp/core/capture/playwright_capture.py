...
class PlaywrightCapture:
    ...
    async def capture(
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install 'caskmcp[playwright]'"
            ) from e
    ...
    def _load_script_runner(self, script_path: str) -> Callable[[Any, Any], Awaitable[None]]:
        ...
        spec = importlib.util.spec_from_file_location("caskmcp_capture_script", path)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python -m caskmcp.core.capture.playwright_capture <url> <allowed_host>")
        sys.exit(1)
