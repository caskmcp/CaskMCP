...
class ContractCompiler:
    ...
    def compile(
        ...
        if capture_id:
            spec["info"]["x-caskmcp"]["capture_id"] = capture_id
        if scope:
            spec["info"]["x-caskmcp"]["scope"] = scope.name
    ...
    def _build_operation(self, endpoint: Endpoint) -> dict[str, Any]:
        ...
        if endpoint.is_state_changing:
            operation["x-caskmcp"]["state_changing"] = True
        if endpoint.is_auth_related:
            operation["x-caskmcp"]["auth_related"] = True
        if endpoint.has_pii:
            operation["x-caskmcp"]["has_pii"] = True
