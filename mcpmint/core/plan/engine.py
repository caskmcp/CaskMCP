...
from mcpmint.core.approval.snapshot import load_snapshot_digest
from mcpmint.core.toolpack import load_toolpack, resolve_toolpack_paths
from mcpmint.models.plan import (
    PlanArtifactInfo,
    PlanBaselineInfo,
    PlanChanges,
    PlanEvidence,
...
from mcpmint.utils.digests import canonical_json_bytes, load_artifact_payload
...
def build_plan(
    *,
    ...
    if lockfile_path is None or not lockfile_path.exists():
        raise ValueError("lockfile missing; run mcpmint approve sync")
...
def _resolve_baseline(
    *,
    ...
    if not lockfile_snapshot_dir or not lockfile_snapshot_digest:
        raise ValueError("baseline snapshot missing; run mcpmint approve snapshot")
    ...
    if digest != lockfile_snapshot_digest:
        raise ValueError("baseline snapshot digest mismatch; re-run mcpmint approve snapshot")
