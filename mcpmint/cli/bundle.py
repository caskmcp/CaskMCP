...
from mcpmint.core.plan.engine import build_plan, render_plan_json, render_plan_md
from mcpmint.core.toolpack import load_toolpack
from mcpmint.utils.config import build_mcp_config_payload, render_config_payload
    ...
    *,
    ...
    with zipfile.ZipFile(
        ...
        for rel_path in sorted(files):
            ...
            mode = EXEC_MODE if rel_path in {"entrypoint.sh", "mcpmint.run"} else FILE_MODE
...
def _collect_toolpack_files(root: Path) -> list[str]:
    ...
    for path in root.rglob("*"):
        ...
        if rel.parts[:2] == (".mcpmint", "reports"):
            continue
...
def _render_run_md(_toolpack_id: str) -> str:
    return (
        "# MCPMint Bundle\n\n"
        "Run this toolpack:\n\n"
        "  mcpmint run --toolpack ./toolpack.yaml\n"
    )
