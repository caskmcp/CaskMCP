"""Meta MCP server that exposes MCPMint itself as tools.
...
from mcpmint.core.approval import ApprovalStatus, LockfileManager
from mcpmint.core.audit import AuditLogger, MemoryAuditBackend
from mcpmint.core.enforce import Enforcer
...
class MCPMintMetaMCPServer:
    """MCP server that exposes MCPMint governance tools.
    ...
    def __init__(
        ...
        Args:
            artifacts_dir: Directory containing MCPMint artifacts
            tools_path: Path to tools.json (overrides artifacts_dir)
            policy_path: Path to policy.yaml (overrides artifacts_dir)
            lockfile_path: Path to lockfile (default: ./mcpmint.lock.yaml)
        """
        ...
        # Create MCP server
        self.server = Server("mcpmint-meta")

        # Register handlers
        self._register_handlers()

        logger.info("Initialized MCPMint Meta MCP server")
    ...
    def _register_handlers(self) -> None:
        ...
        async def handle_list_tools() -> list[types.Tool]:
            ...
            return [
                types.Tool(
                    name="mcpmint_list_actions",
                    description="List all available actions from the MCPMint manifest with their risk tiers and approval status",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "filter_risk": {
                ...
                types.Tool(
                    name="mcpmint_get_approval_status",
                ...
                types.Tool(
                    name="mcpmint_list_pending_approvals",
                ...
                types.Tool(
                    name="mcpmint_get_action_details",
        ...
        if not manager.exists():
            return [types.TextContent(
                ...
                text=json.dumps({
                    "action": action_name,
                    "status": "no_lockfile",
                    "message": "No lockfile found - run 'mcpmint approve sync' first",
                })
            )]
    ...
    async def _list_pending_approvals(self) -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
        ...
        if not manager.exists():
            return [types.TextContent(
                type="text",
                text=json.dumps({
                    "error": "No lockfile found - run 'mcpmint approve sync' first",
                })
            )]
...
def run_meta_server(
    ...
    """Run the MCPMint Meta MCP server.
