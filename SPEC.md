# MCPMint Architecture Specification

**Version:** 1.0.0
**Status:** Draft
**Last Updated:** 2025-02-04

MCPMint is an "action surface compiler" that transforms observed web/API traffic into safe, versioned, agent-ready tools with drift detection and enforcement guardrails.
...
mcpmint/
├── __init__.py
├── __main__.py                 # Entry point: python -m mcpmint
│
├── cli/                        # Click CLI commands
│   ├── __init__.py
│   ├── main.py                 # CLI group and common options
│   ├── capture.py              # mcpmint capture
│   ├── compile.py              # mcpmint compile
│   ├── drift.py                # mcpmint drift
│   ├── serve.py                # mcpmint serve
│   └── enforce.py              # mcpmint enforce
│
├── core/
│   ├── __init__.py
│   │
...
mcpmint capture import path/to/traffic.har \
...
mcpmint capture record https://example.com \
  --allowed-hosts api.example.com \
  --headless \
  --duration 30
...
| `--output` | No | Output directory (default: `./.mcpmint/captures/`) |
...
| `--output` | No | Root output dir (default: `./.mcpmint`) |
| `--print-mcp-config` | No | Print Claude Desktop MCP config snippet |

**Output:**
- Creates `.mcpmint/toolpacks/<toolpack_id>/toolpack.yaml`
- Copies compiled artifacts under `.mcpmint/toolpacks/<toolpack_id>/artifact/`
- Creates pending lockfile at `.mcpmint/toolpacks/<toolpack_id>/lockfile/mcpmint.lock.pending.yaml`
...
| `--output` | No | Output directory (default: `./.mcpmint/artifacts/`) |
...
| `--artifacts` | No | Artifacts directory (default: `./.mcpmint/artifacts/`) |
...
| `--audit-log` | No | Path for audit log (default: `./.mcpmint/audit.jsonl`) |
| `--dry-run` | No | Evaluate but don't execute |

### 6.7 `mcpmint mcp serve`

Expose compiled tools over MCP stdio transport.

```bash
# Explicit artifacts
mcpmint mcp serve --tools ./artifacts/tools.json --policy ./artifacts/policy.yaml

# Toolpack-resolved artifacts
mcpmint mcp serve --toolpack ./.mcpmint/toolpacks/<toolpack_id>/toolpack.yaml
```
...
```python
from mcpmint.core.capture.base import CaptureAdapter
...
# Register
from mcpmint.core.capture import register_adapter
...
```python
from mcpmint.core.scope import ScopeProvider
...
# Register
from mcpmint.core.scope import register_provider
...
```python
from mcpmint.core.drift import DriftClassifier
...
# Register
from mcpmint.core.drift import register_classifier
...
```python
from mcpmint.core.audit import AuditBackend
...
# Register
from mcpmint.core.audit import register_backend
...
# Register
from mcpmint.core.capture import register_redactor
...
These safety defaults are built into MCPMint's core and **cannot be bypassed accidentally**. Any attempt to disable them requires explicit, documented override.
...
When an override is used, MCPMint:
...
```json
{
  ...
  "command": "mcpmint capture import foo.har --no-redact",
...
**Deliverable:** `mcpmint capture import` works end-to-end.
...
**Deliverable:** `mcpmint compile` produces contract + tools.
...
**Deliverable:** `mcpmint drift` with CI exit codes.
...
**Deliverable:** `mcpmint enforce` blocks unauthorized requests.
...
**Deliverable:** `mcpmint mint <start_url> -a <host>` creates a usable toolpack and `mcpmint mcp serve --toolpack ...` works without manual artifact path lookup.
...
info:
  title: "Example API"
  version: "1.0.0"
  description: "Generated by MCPMint"
  x-mcpmint:
    capture_id: "cap_20240204_abc123"
    scope: "first_party_only"
